...
本文档说明如何为 SummitLink 配置腾讯云 CDN 加速，覆盖加速域名设置、COS 联合加速、缓存规则、HTTPS 证书及回源鉴权。
...
| 源站类型 | 配置值 | 备注 |
|---------|--------|------|
| COS 域名 | `https://unsummit-cn-1234567890.cos.ap-beijing.myqcloud.com` | 替换为实际 Bucket + Region |
| 自定义源站 | `https://summitlink.up.railway.app` | 直接回源到 Railway（非对象存储场景） |

> **推荐方案**：图片存 COS，CDN 以 COS 为源站；API 仍走 Railway。
...
用户请求 cdn.summitlink.com/uploads/xxx.jpg
  ...
  腾讯云 CDN 节点（就近命中缓存）
        ↓ 未命中
  COS Bucket（unsummit-cn-1234567890, ap-beijing）
        ↓
  返回图片并在 CDN 节点缓存
```
...
1. 在 COS Bucket 控制台开启 **自定义 CDN 域名**，绑定 `cdn.summitlink.com`
2. 在 CDN 域名管理中将源站配置为该 COS Bucket 的默认域名
3. 在后端环境变量中设置：
   ```env
   COS_BUCKET=unsummit-cn-1234567890
   COS_REGION=ap-beijing
   COS_CDN_DOMAIN=https://cdn.summitlink.com
   ```
4. 上传图片后，CN 存储适配器会自动优先返回 CDN URL
...
防止未经授权的直接访问 COS 源站（绕过 CDN）：

1. 在 COS Bucket → **防盗链** 中开启 **Referer 白名单**：
   ```
...
2. 在 CDN 控制台 → **访问控制** → **URL 鉴权** 中启用 A 类型鉴权：
   - 生成鉴权 Key（保存到环境变量 `COS_CDN_AUTH_KEY`）
   - 有效时长建议设为 1800 秒

3. COS Bucket 权限改为**私有**，仅允许 CDN 回源 IP 段访问（在 Bucket Policy 中配置）
...
- 开启 **图片处理**（腾讯云数据万象 / CI）：自动 WebP 转换、缩略图
