...
### 🔴 生产禁止项（上线前必须解决）
- [ ] `mock-pay` 接口必须在生产环境关闭或删除（路由：`POST /api/expeditions/orders/:id/mock-pay`）
- [ ] `GET /api/admin/sms-codes` 明文验证码接口必须在生产环境关闭
- [ ] 高德地图Key不得硬编码在前端HTML文件中（需迁移至后端代理）
...
| 0.2 | 关闭/保护mock-pay和sms-codes生产接口 | P0 | 待做 | |
| 0.3 | 高德Key迁移至后端代理 | P0 | 待做 | |
...
| `AMAP_KEY` | ⚠️ 前端暴露 | 需迁移后端代理 |
| `AMAP_SECURITY_CODE` | ⚠️ 前端暴露 | 同上 |
...
*文档版本：v1.0 | 最后更新：2026-04-26 | 维护人：@gaoshanyindi*
