...
| `RESEND_FROM` | 官网线索邮件必需 | `no-reply@unsummit.cn` | `SummitLink <no-reply@unsummit.cn>` | 发件人地址。需在 Resend 完成域名验证 |
| `LEADS_NOTIFY_EMAIL` | 官网线索管理员通知必需 | — | `hello@unsummit.cn` | 官网 4 个公开表单写库后通知的运营收件人。未配置时使用 `ADMIN_EMAIL` 兜底 |
| `ADMIN_EMAIL` | 兜底 | — | `hello@unsummit.cn` | `LEADS_NOTIFY_EMAIL` 未配置时的线索通知收件人 |
...
LEADS_NOTIFY_EMAIL=hello@unsummit.cn
AMAP_KEY=<高德 Key>
