...
// 章节标记（匹配 .env.example 中以 "# ──" 开头的章节标题行）：
//   含 "必填" 的章节标题 → 接下来的变量为必填
//   含 "可选" 的章节标题 → 接下来的变量为可选
//
// 注意：出现在第一个章节标题之前的变量默认视为可选；
// .env.example 应始终以 "# ── 必填" 章节标题开头以确保行为明确。
const SECTION_HEADER_RE = /^#\s*─+\s*/; // 以 # ── 开头的章节标题行
...
for (const raw of lines) {
  ...
  // 纯注释行：检查是否为章节标题，并保留为下一个变量的行级注释
  if (line.startsWith('#')) {
    const commentText = line.replace(/^#+\s*/, '');
    // Only toggle section state on dedicated section-header lines (e.g. "# ── 必填 ──")
    // to avoid false positives from inline comments that happen to contain these words.
