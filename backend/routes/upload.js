...
const { CLOUD_STORAGE_ENABLED, uploadFilesToStorage, uploadBufferToStorage } = require('../middleware/storageUpload');
...
router.post('/video', uploadLimiter, auth, (req, res) => {
  uploadVideo.single('file')(req, res, async (err) => {
    if (err) return res.status(400).json({ error: err.message || '只允许上传视频文件（mp4/mov/m4v）' });
    if (!req.file) return res.status(400).json({ error: '未收到视频文件' });
    try {
      // 视频必须进入对象存储（腾讯云 COS）
      if (CLOUD_STORAGE_ENABLED) {
        await uploadFilesToStorage([req.file], uploadDir, 'videos');
      }
      const url = req.file.storageUrl || ('/uploads/' + req.file.filename);
...
router.post('/gpx', uploadLimiter, auth, (req, res) => {
  uploadGpx.single('file')(req, res, async (err) => {
    ...
    try {
      ...
      // GPX 原始文件必须进入对象存储（腾讯云 COS），保留原始轨迹文件
      let url = null;
      if (CLOUD_STORAGE_ENABLED) {
        url = await uploadBufferToStorage(buffer, req.file.originalname, 'gpx', 'application/gpx+xml');
      }
      // Clean up temp file
      fs.unlink(safeGpxPath, () => {});
      res.json({ name: trackName, points, count: points.length, url });
    } catch (e) {
      fs.unlink(safeGpxPath, () => {});
