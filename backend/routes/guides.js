...
router.get('/my/finance', guideReadLimiter, auth, async (req, res) => {
  try {
    ...
    // 待结算（paid 状态的订单，尚未提现的部分）
    const pendingRows = await prisma.$queryRaw`
      SELECT COALESCE(SUM(amount), 0) as total
      FROM guide_service_orders
...
router.get('/my/dashboard', guideReadLimiter, auth, async (req, res) => {
  try {
    ...
    // 路线浏览量（guide_services view_count 合计）
    const viewRows = await prisma.$queryRaw`
      SELECT COALESCE(SUM(view_count), 0) as total
      FROM guide_services WHERE guide_id = ${guide.id} AND status != 'deleted'
    `.catch(() => [{ total: 0 }]);
    const totalViews = Number(viewRows[0]?.total || 0);

    // 复购率（下过 2 次及以上订单的用户数 / 总用户数）
  ...
  try {
    ...
    if (authHeader.startsWith('Bearer ')) {
      try {
        ...
        const [guide] = await prisma.$queryRaw`SELECT id FROM guides WHERE user_id = ${decoded.id}`;
        if (guide && guide.id === guideId) isOwner = true;
      } catch (_) {}
    }
    ...
    if (isOwner) {
      ...
      if (type) {
        services = await prisma.$queryRaw`
          SELECT * FROM guide_services WHERE guide_id = ${guideId} AND status = 'active' AND type = ${type}
          ORDER BY created_at DESC
        `;
      } else {
        services = await prisma.$queryRaw`
          SELECT * FROM guide_services WHERE guide_id = ${guideId} AND status = 'active'
          ORDER BY created_at DESC
        `;
      }
    }
...
router.post('/:guideId/services', guideWriteLimiter, auth, async (req, res) => {
  try {
    ...
    // 幂等检测：30 秒内相同 title + start_date 的服务视为重复提交
...
router.post('/my/routes', guideWriteLimiter, auth, async (req, res) => {
  try {
    ...
    `.catch(() => [null]);
    if (!route) return res.json({ success: true, id: null });
    res.json(route);
  } catch (e) {
...
// PUT /api/guides/my/routes/:id — 更新路线草稿
router.put('/my/routes/:id', guideWriteLimiter, auth, async (req, res) => {
  try {
    const [guide] = await prisma.$queryRaw`SELECT id FROM guides WHERE user_id = ${req.user.id}`;
...
// POST /api/guides/routes/:id/submit — 提交路线审核
router.post('/routes/:id/submit', guideWriteLimiter, auth, async (req, res) => {
  try {
    const [guide] = await prisma.$queryRaw`SELECT id FROM guides WHERE user_id = ${req.user.id}`;
