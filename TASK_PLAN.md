# 📋 SummitLink 任务分工总表
> **最后更新**：2026-05-18（全栈审计基线 + 第八～十二批 PR 规划）
...
### 🔴 第八批 — 审计阻塞修复（Blockers，上线必须）
> 来源：`docs/AUDIT_2026-05-18.md` §7.1 全部 7 项
...
| PR-49 | 装备清单 PDF 导出（`routes/climbingLog.js:108` 从占位到真实 PDF） | §3.1 | 无 | ⬜ |
| PR-50 | 电子护照 QR 升级（`routes/passport.js:189` 真实二维码） | §3.1 | 无 | ⬜ |
| PR-51 | AI Assistant 配置 `OPENAI_API_KEY` 后去 Mock 回退（接入 GPT-4o） | §3.1 | 无 | ⬜ |
...
3. **PR-35** — nginx HTTPS/TLS（裸跑 HTTP，无法生产）
